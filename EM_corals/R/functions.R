...
composition <- function(phy, col, legend=T) {
  ...
  if (legend==T) {
    ...
    legend(x=par("usr")[2]-0.03, y=par("usr")[4], xjust=0, yjust=0.1, bty="n", xpd=T, cex=0.6, 
           pt.cex=0.6, legend=levels(samdat$Site), fill=rainbow(5), y.intersp=0.7, 
           x.intersp=0.3)
  }
  # Add grouping bars for Site
  for (i in 1:length(sitebreaks)) {
    lines(c(0, 1), c(sitebreaks[i], sitebreaks[i]), lty=2, lwd=0.25)
    rect(1.01, sitebreaks[i], 1.04, sitebreaks[i+1], col=rep(rainbow(5), 3)[i], 
         lwd=0.25, xpd=T)
  }
  ...
  for (i in 1:length(spbreaks)) {
    ...
    text(1.03, (spbreaks[i] + spbreaks[i+1]) / 2, xpd=T, pos=4, cex=0.6, font = 3,
         labels=paste(samdat$Species[which(duplicated(samdat$Species)==F)][i], sep=""))
  }
}

makenet <- function(physeq, thresh, type) {
  require(phyloseq)
  require(reshape2)
  require(igraph)
  ...
  nodes <- nodes[nodes$id %in% c(as.character(edges$source), as.character(edges$target)), ]
  # Create network
  if (type=="igraph") net <- graph_from_data_frame(d=edges, vertices=nodes, directed=F)
  return(net)
}
...
otubarplot <- function(phy, main) {
  ...
  bars <- barplot(typerelabund, col=rainbow(ntaxa(phy)), las=1, space=0.1, xaxs="i",
                  xlab="", ylab="", xaxt="n")
  text(bars, 0, labels=samples, xpd=T, srt=90, adj=c(1,0.5))
...
perms <- function(phy, group, trt) {
  groups <- factor(data.frame(sample_data(phy))[, group])
  ...
  for (i in 1:nlevels(factor(groups))) {
    ...
    md <- vegan::meandist(braydist, samdat[, trt])
    ...
    if (nlevels(factor(as(sample_data(phy.group), "data.frame")[, trt])) > 1) {
      permanova <- vegan::adonis(braydist ~ get(trt), data=samdat, permutations=99999)
